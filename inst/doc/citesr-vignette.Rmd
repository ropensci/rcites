...
Below, we assume that the user already possesses a token. For illustrative purposes  we will use the generic token value `8QW6Qgh57sBG2k0gtt` just as on the API documentation, but each user should obtain it's own token to run the code below (see
>>>>>>> master
...
requests. They are three different ways to use the token in ``rcites``:
...
current R session (meaning login wille be required in future R sessions);
