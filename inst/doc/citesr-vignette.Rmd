...
<<<<<<< HEAD
Below, we assume that the user already possesses a token and that its
value is `8QW6Qgh57sBG2k0gtt` just as on the API documentation (see
=======
Below, we assume that the user already possesses a token. For iluastrative purposes  we will use the generic token value `8QW6Qgh57sBG2k0gtt` just as on the API documentation, but each user should obtain it's own token to run the code below (see
>>>>>>> master
...
requests. They are three different ways to use the token in ``citesr``:
...
<<<<<<< HEAD
for the current R session (meaning a login will be needed the next session);
=======
for the current R session (meaning a new login will be needed for the next session);
...
=======
Also if the taxon is not listed a warning message should come up to assert so:
>>>>>>> master
