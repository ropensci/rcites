...
We can also retrieve current EU annex listings, SRG opinions, and EU suspensions.

```{r}
spp_eu_legislation(taxon_id = "4521",  verbose = FALSE)
```

Note that the time scope of legislation values can be specified among `current`, `historic` and `all`. Default is set to `current`.
