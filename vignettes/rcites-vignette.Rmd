...
2. use `sppplus_login()`, *i.e.* interactively set `SPECIESPLUS_TOKEN` up for the
...
so, the user should first call `spp_taxonconcept()` and provide the scientific
...
```{r qtxla2}
res2 <- spp_taxonconcept(query_taxon = "Loxodonta africana")
res2
```
...
```{r qtxhs}
res <- spp_taxonconcept(query = "Homo Sapiens")
```
...
leg2 <- spp_cites_legislation(taxon_id = '3210', simplify = T)
```
...
```{r}
ref2 <- spp_references(taxon_id = '3210', simplify = T)
...
res <- lapply(vc_sp, function(x) spp_taxonconcept(query_taxon = x))
do.call(rbind, res)
```
...
res <- lapply(vc_id,
    function(x) unlist(
      spp_eu_legislation(taxon_id = x, type = "listings", simplify = TRUE)
    )[1:6]
    )
do.call(rbind, res)
```
