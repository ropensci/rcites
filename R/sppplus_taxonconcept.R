...
#' @importFrom XML xmlToList xmlRoot xmlParse xmlToDataFrame xmlName
#'
#' @export
#' @examples
...
sppplus_taxonconcept <- function(cnx, query = "Loxodonta africana", appendix_only = TRUE) {
    ...
    temp2 <- xmlRoot(xmlParse(temp))
    if (xmlToList(unlist(temp2[[1L]][[3L]]))$text == "0") {
        message("species not listed in CITES")
    } else {
        if (appendix_only) {
            temp3 <- xmlToDataFrame(unlist(temp2[[2L]]["taxon-concept"]))
            temp3 <- temp3[c(1, 2, 8)]
