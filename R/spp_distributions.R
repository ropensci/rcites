...
spp_distributions <- function(taxon_id, language = "en", raw = FALSE, token = NULL, verbose = TRUE) {

    if (length(taxon_id) > 1) {
      out <- lapply(taxon_id, spp_distributions, language = language, raw = raw, token = token)
      out <- rcites_combine_lists(out, taxon_id)
    } else {
      # token check
      if (is.null(token))
